...
from external.caribbeancom_pr import api
...
class Test(TestCase):
    ...
    def test_get_item_2(self):
        ...
        self.assertEqual(u"Debut 〜現役人気No.1女優、上原亜衣解禁〜", item.title)
        self.assertEqual(u"日本AV界にその名を轟かす現役人気No.1女優・上原亜衣がデビューをしちゃうんです〜！しっ、しかも、まだ世に出回"
                         u"っていない未公開映像付きの完全版！男が一番エロいと感じる女のカラダを持つ亜衣ちゃんのマンコも一味違かった！ぷ"
                         u"にゅぷにゅした綺麗なもっちりおマンコに小ぶりに輝くクリトリス。その素晴らしきマンコを2本の指でシャッフルすると"
                         u"、大量の愛液と潮を吹きまくっちゃいます！", item.description)
        self.assertEqual(u"https://smovie.caribbeancompr.com/moviepages/032715_157/images/main_b.jpg", item.poster_url)
        self.assertEqual(u"https://smovie.caribbeancompr.com/moviepages/032715_157/images/l_l.jpg", item.background_url)
        # self.assertEqual(u"https://www.caribbeancompr.com/search_act/6706/1.html", item.actor_url)
        self.assertEqual(u"上原亜衣", item.actor_name)
        # self.assertEqual(6706, item.actor_id)
        # self.assertEqual(u"https://www.caribbeancom.com/box/search_act/6706/images/top.jpg", item.actor_large_picture_url)
        # self.assertEqual(u"https://www.caribbeancom.com/images/actress/50x50/actor_6706.jpg", item.actor_small_picture_url)
        self.assertEqual(u"https://smovie.caribbeancompr.com/sample/movies/032715_157/480p.mp4", item.sample_video_url)
        self.assertEqual(datetime.date(2015, 3, 27), item.upload_date)
        self.assertEqual(datetime.time(1, 5, 39), item.duration)
        self.assertEqual(3939, item.duration_in_seconds)
        self.assertEqual(None, item.series_name)
        # self.assertEqual(960, item.series_id)
        # self.assertEqual(u"https://www.caribbeancom.com/series/960/index.html", item.series_url)
        self.assertEqual(14, len(item.tags))
        self.assertEqual(u"AV女優", item.tags[0].name)
        self.assertEqual(u"1", item.tags[0].slug)
        self.assertEqual(u"https://www.caribbeancompr.com/listpages/1_1.html", item.tags[0].url)
        # self.assertEqual(7, len(item.genres))
    ...
    def test_has_valid_id(self):
        self.assertEqual(True, api.has_valid_id("caribpr-123123-233"))
        self.assertEqual(True, api.has_valid_id("Caribpr-123123-233"))
        self.assertEqual(True, api.has_valid_id("Caribpr-123123-233-asd"))
        self.assertEqual(True, api.has_valid_id("Caribpr-123123-233-FHD"))
        self.assertEqual(False, api.has_valid_id("Caribpr-12123-233-FHD"))
        self.assertEqual(True, api.has_valid_id("Caribpr-123123-23123123-FHD"))
        self.assertEqual(True, api.has_valid_id("Caribpr-123123-1-FHD"))
        self.assertEqual(True, api.has_valid_id("Caribbeanpr-123123-1-FHD"))
        self.assertEqual(True, api.has_valid_id("Caribbeancompr-123123-1-FHD"))
        self.assertEqual(False, api.has_valid_id("Caribbpr-123123-1-FHD"))
        self.assertEqual(False, api.has_valid_id("Caribbeancpr-123123-1-FHD"))
